...
This example application shows how to use the SST26 driver to perform block operations on<br /> the On-Board SST26 Flash memory in SQI mode.
...
This application uses the SST26 driver to Erase/Write/Read on the On-Board SST26<br /> Flash memory using the SQI peripheral library configured in SQI mode. The<br /> application consists of APP\_SST26\_Tasks\(\) which is called through SYS\_Tasks\(\)<br /> routine.
...
Path of the application within the repository is<br /> **apps/driver/sqi\_flash/sst26/sst26\_sqi\_read\_write/firmware**.
...
2.  LED is turned ON when the data read from SST26 flash matches with the data written.
