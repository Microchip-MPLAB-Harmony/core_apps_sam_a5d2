...
{"0": {
    ...
    "doc": "FreeRTOS basic",
    "title": "Running the Application",
    "content": ". | Build the application using its IDE | Copy the output binary (named ‘harmony.bin’) onto the SD Card (Refer to the ‘Setting up hardware’ section above for setting up the SD card) | Insert the SD card into SDMMC slot on the board (Refer to the ‘Setting up hardware’ section for the correct SDMMC slot) | Reset the board to run the application | The LED indicates the success or failure. | The LED toggles on success i.e. each time when the Task2 or Task3 receives a valid message | . | . Refer to the following table for LED name: . | Board | LED Name | . | SAMA5D2 Xplained Ultra Evaluation Kit | RGB_LED(Blue) | . |   |   | . ",
