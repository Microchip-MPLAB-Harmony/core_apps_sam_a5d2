...
{"0": {
    ...
    "content": ". | SD Card with FAT32 file system to store the AT91Bootstrap bootloader and the final application | . Setting up SAMA5D2 Xplained Ultra Evaluation Kit . Setting up the SD Card . | Download harmony MPU bootstrap loader from this location | Copy the downloaded boot loader binary( boot.bin) onto the SD card | Copy the application (harmony.bin) from “firmware/sam_a5d2_xult.X/dist/sam_a5d2_xult/production” or “firmware/sam_a5d2_xult_freertos.X/dist/sam_a5d2_xult_freertos/production” for MPLAB X project onto the SD card, available after a successful build of the application (Refer to the ‘Running the Application’ section below) NOTE: For IAR project, you can find harmony.bin in the “firmware/sam_a5d2_xult_freertos_iar.IAR/Debug/Exe” folder | . Setting up the board . | Insert the FAT32 formatted SD card with the boot.bin and harmony.bin (Refer to the ‘Running the Application’ section below) to J19 (SDMMC1) | Short jumper JP2 (DEBUG_DIS) | Connect a USB to UART FTDI cable to J1 (DEBUG) to open a serial terminal | Connecting a micro-USB cable to either J23 or J14 powers the board | . Setting up SAMA5D27-SOM1-EK1 Evaluation Kit . Setting up the SD Card . | Download harmony MPU bootstrap loader from this location | Copy the downloaded boot loader binary( boot.bin) onto the SD card | Copy the application (harmony.bin) from “firmware/sam_a5d27_som1_ek_freertos.X/dist/sam_a5d27_som1_ek_freertos/production” onto the SD card, available after a successful build of the application (Refer to the ‘Running the Application’ section below) | . Setting up the board . | Insert the FAT32 formatted SD card with the boot.bin and harmony.bin (Refer to the ‘Running the Application’ section below) to J12 | Connecting a micro-USB cable to J10 both powers the board as well as provides a virtal COM port to open a serial terminal | . Setting up SAMA5D27-WLSOM1-EK1 Evaluation Kit . Setting up the SD Card . | Download harmony MPU bootstrap loader from this location | Copy the downloaded boot loader binary( boot.bin) onto the SD card | Copy the application (harmony.bin) from “firmware/sam_a5d27_wlsom1_ek1_freertos.X/dist/sam_a5d27_wlsom1_ek1_freertos/production” onto the SD card, available after a successful build of the application (Refer to the ‘Running the Application’ section below) | . Setting up the board . | Insert the FAT32 formatted SD card with the boot.bin and harmony.bin (Refer to the ‘Running the Application’ section below) to J9 | Connect a USB to UART FTDI cable to J26 (DEBUG FTDI) to open a serial terminal | Connecting a micro-USB cable to J10 powers the board | Press the SW3 (nSTART_SOM) button to power the ATSAMA5D27-WLSOM1 System-On-Module | . ",
    "url": "http://localhost:4000/core_apps_sam_a5d2/apps/fs/sqi_flash_fat/readme.html#addtional-hardware-required",
    "relUrl": "/apps/fs/sqi_flash_fat/readme.html#addtional-hardware-required"
  },"177": {
    "doc": "FAT filesystem using SQI Flash Media",
    "title": "Running the Application",
